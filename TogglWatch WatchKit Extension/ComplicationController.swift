...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    private func templateForRunningTEWithInfo(complication: CLKComplication, start: Date, description: String?) -> CLKComplicationTemplate?
    {
        ...
        case .modularLarge:
            ...
            if let projectName = UserDefaultsConfig.runningTEProjectName {
                let projectnameTextProvider = CLKSimpleTextProvider(text: projectName)
                modularLargeTemplate.body2TextProvider = CLKTextProvider(format: "• %@", projectnameTextProvider)
            }
